...
5. [How do I use bootstrap.dat with pod?](#Importing)
...
### 5. How do I use bootstrap.dat with pod?

pod comes with a separate utility named `addblock` which can be used to import
...
1. Stop pod if it is already running.  This is required since addblock needs to
   access the database used by pod and it will be locked if pod is using it.
