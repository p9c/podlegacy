...
func makeTestOutput(r *rpctest.Harness, t *testing.T,
	amt utils.Amount) (*ecc.PrivateKey, *wire.OutPoint, []byte, error) {
	...
	// spend.
	a, err := utils.NewAddressPubKey(key.PubKey().SerializeCompressed(), r.ActiveNet)
	if err != nil {
		return nil, nil, nil, err
	}
...
func TestBIP0113Activation(t *testing.T) {
	...
	const outputValue = utils.SatoshiPerBitcoin
...
func createCSVOutput(r *rpctest.Harness, t *testing.T,
	numSatoshis utils.Amount, timeLock int32,
	...
	p2shAddr, err := utils.NewAddressScriptHash(csvScript, r.ActiveNet)
	if err != nil {
		return nil, nil, nil, err
	}
...
func TestBIP0068AndBIP0112Activation(t *testing.T) {
	...
	const (
		outputAmt         = utils.SatoshiPerBitcoin
	...
	for txVersion := int32(0); txVersion < 3; txVersion++ {
		...
		txns := []*utils.Tx{utils.NewTx(spendingTx)}
		block, err := r.GenerateAndSubmitBlock(txns, -1, time.Time{})
		if err != nil {
			t.Fatalf("unable to submit block: %v", err)
		}
	...
	for i, test := range tests {
		...
		if !test.accept {
			txns := []*utils.Tx{utils.NewTx(test.tx)}
			_, err := r.GenerateAndSubmitBlock(txns, -1, time.Time{})
			if err == nil {
