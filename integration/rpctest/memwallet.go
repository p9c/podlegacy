...
type memWallet struct {
	...
	addrs map[uint32]utils.Address
...
func newMemWallet(net *chaincfg.Params, harnessID uint32) (*memWallet, error) {
	...
	addrs := make(map[uint32]utils.Address)
...
func (m *memWallet) IngestBlock(height int32, header *wire.BlockHeader, filteredTxns []*utils.Tx) {
...
func (m *memWallet) evalOutputs(outputs []*wire.TxOut, txHash *chainhash.Hash,
	...
	for i, output := range outputs {
		...
		for keyIndex, addr := range m.addrs {
			...
			m.utxos[op] = &utxo{
				value:          utils.Amount(output.Value),
				keyIndex:       keyIndex,
...
func (m *memWallet) newAddress() (utils.Address, error) {
	index := m.hdIndex
	...
	err = m.rpc.LoadTxFilter(false, []utils.Address{addr}, nil)
	if err != nil {
		return nil, err
	}
...
func (m *memWallet) fundTx(tx *wire.MsgTx, amt utils.Amount,
	feeRate utils.Amount, change bool) error {
	...
	for outPoint, utxo := range m.utxos {
		...
		reqFee := utils.Amount(txSize * int(feeRate))
...
func (m *memWallet) SendOutputs(outputs []*wire.TxOut,
	feeRate utils.Amount) (*chainhash.Hash, error) {
...
func (m *memWallet) CreateTransaction(outputs []*wire.TxOut,
	feeRate utils.Amount, change bool) (*wire.MsgTx, error) {
	...
	var outputAmt utils.Amount
	for _, output := range outputs {
		outputAmt += utils.Amount(output.Value)
		tx.AddTxOut(output)
	}
...
//
// This function is safe for concurrent access.
func (m *memWallet) ConfirmedBalance() utils.Amount {
	m.RLock()
	defer m.RUnlock()
...
func keyToAddr(key *ecc.PrivateKey, net *chaincfg.Params) (utils.Address, error) {
	serializedKey := key.PubKey().SerializeCompressed()
	pubKeyAddr, err := utils.NewAddressPubKey(serializedKey, net)
	if err != nil {
		return nil, err
	}
