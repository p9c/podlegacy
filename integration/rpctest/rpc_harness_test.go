...
func testSendOutputs(r *Harness, t *testing.T) {
	genSpend := func(amt utils.Amount) *chainhash.Hash {
	...
	// input.
	txid := genSpend(utils.Amount(5 * utils.SatoshiPerBitcoin))
	...
	txid = genSpend(utils.Amount(500 * utils.SatoshiPerBitcoin))
	blockHashes, err = r.Node.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testGenerateAndSubmitBlock(r *Harness, t *testing.T) {
	...
	output := wire.NewTxOut(utils.SatoshiPerBitcoin, pkScript)

	const numTxns = 5
	txns := make([]*utils.Tx, 0, numTxns)
	for i := 0; i < numTxns; i++ {
		...
		txns = append(txns, utils.NewTx(tx))
	}
...
func testMemWalletReorg(r *Harness, t *testing.T) {
	...
	expectedBalance := utils.Amount(250 * utils.SatoshiPerBitcoin)
	...
	// chain.
	expectedBalance = utils.Amount(0)
	walletBalance = harness.ConfirmedBalance()
	if expectedBalance != walletBalance {
		t.Fatalf("wallet balance incorrect: expected %v, got %v",
			expectedBalance, walletBalance)
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := utils.Amount(50 * utils.SatoshiPerBitcoin)
...
func TestHarness(t *testing.T) {
	...
	// outputs.
	expectedBalance := utils.Amount(numMatureOutputs * 50 * utils.SatoshiPerBitcoin)
