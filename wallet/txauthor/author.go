...
import (
	...
	"github.com/parallelcointeam/pod/wallet/internal/txsizes"
)
...
// InputSourceError.
type InputSource func(target utils.Amount) (total utils.Amount, inputs []*wire.TxIn,
	inputValues []utils.Amount, scripts [][]byte, err error)
...
type AuthoredTx struct {
	Tx              *wire.MsgTx
	PrevScripts     [][]byte
	PrevInputValues []utils.Amount
	TotalInput      utils.Amount
...
func NewUnsignedTransaction(outputs []*wire.TxOut, relayFeePerKb utils.Amount,
...
func AddAllInputScripts(tx *wire.MsgTx, prevPkScripts [][]byte, inputValues []utils.Amount,
...
func spendWitnessKeyHash(txIn *wire.TxIn, pkScript []byte,
	...
	p2wkhAddr, err := utils.NewAddressWitnessPubKeyHash(pubKeyHash, chainParams)
	if err != nil {
		return err
	}
