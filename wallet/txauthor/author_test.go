...
func p2pkhOutputs(amounts ...utils.Amount) []*wire.TxOut {
...
func makeInputSource(unspents []*wire.TxOut) InputSource {
	...
	currentInputValues := make([]utils.Amount, 0, len(unspents))
	f := func(target utils.Amount) (utils.Amount, []*wire.TxIn, []utils.Amount, [][]byte, error) {
		for currentTotal < target && len(unspents) != 0 {
			u := unspents[0]
			unspents = unspents[1:]
			nextInput := wire.NewTxIn(&wire.OutPoint{}, nil, nil)
			currentTotal += utils.Amount(u.Value)
			currentInputs = append(currentInputs, nextInput)
			currentInputValues = append(currentInputValues, utils.Amount(u.Value))
		}
...
func TestNewUnsignedTransaction(t *testing.T) {
	tests := []struct {
		...
		Outputs          []*wire.TxOut
		RelayFee         utils.Amount
		ChangeAmount     utils.Amount
