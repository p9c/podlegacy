...
func (p *Pool) DepositScriptAddress(seriesID uint32, branch Branch, index Index) (utils.Address, error) {
	script, err := p.DepositScript(seriesID, branch, index)
	if err != nil {
		return nil, err
	}
...
func (p *Pool) addressFor(script []byte) (utils.Address, error) {
	scriptHash := utils.Hash160(script)
	return utils.NewAddressScriptHashFromHash(scriptHash, p.manager.ChainParams())
}
...
func (p *Pool) DepositScript(seriesID uint32, branch Branch, index Index) ([]byte, error) {
	...
	pks := make([]*utils.AddressPubKey, len(pubKeys))
	for i, key := range pubKeys {
		...
		pks[i], err = utils.NewAddressPubKey(pubkey.SerializeCompressed(),
...
func (p *Pool) addUsedAddr(ns, addrmgrNs walletdb.ReadWriteBucket, seriesID uint32, branch Branch, index Index) error {
	...
	encryptedHash, err := p.manager.Encrypt(waddrmgr.CKTPublic, utils.Hash160(script))
	if err != nil {
...
func (p *Pool) getUsedAddr(ns, addrmgrNs walletdb.ReadBucket, seriesID uint32, branch Branch, index Index) (
	...
	addr, err := utils.NewAddressScriptHashFromHash(hash, mgr.ChainParams())
	if err != nil {
