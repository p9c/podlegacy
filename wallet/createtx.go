...
func makeInputSource(eligible []wtxmgr.Credit) txauthor.InputSource {
	...
	currentTotal := utils.Amount(0)
	currentInputs := make([]*wire.TxIn, 0, len(eligible))
	currentScripts := make([][]byte, 0, len(eligible))
	currentInputValues := make([]utils.Amount, 0, len(eligible))

	return func(target utils.Amount) (utils.Amount, []*wire.TxIn,
		[]utils.Amount, [][]byte, error) {
...
func (s secretSource) GetKey(addr utils.Address) (*ecc.PrivateKey, bool, error) {
...
func (s secretSource) GetScript(addr utils.Address) ([]byte, error) {
	ma, err := s.Address(s.addrmgrNs, addr)
	if err != nil {
		return nil, err
	}
...
func (w *Wallet) txToOutputs(outputs []*wire.TxOut, account uint32,
	minconf int32, feeSatPerKb utils.Amount) (tx *txauthor.AuthoredTx, err error) {
	...
	if tx.ChangeIndex >= 0 && account == waddrmgr.ImportedAddrAccount {
		changeAmount := utils.Amount(tx.Tx.TxOut[tx.ChangeIndex].Value)
...
func validateMsgTx(tx *wire.MsgTx, prevScripts [][]byte, inputValues []utils.Amount) error {
