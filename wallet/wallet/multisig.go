...
func (w *Wallet) MakeMultiSigScript(addrs []utils.Address, nRequired int) ([]byte, error) {
	pubKeys := make([]*utils.AddressPubKey, len(addrs))
...
func (w *Wallet) ImportP2SHRedeemScript(script []byte) (*utils.AddressScriptHash, error) {
	var p2shAddr *utils.AddressScriptHash
	err := walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) error {
		...
		if err != nil {
			...
			if waddrmgr.IsError(err, waddrmgr.ErrDuplicateAddress) {
				...
				p2shAddr, _ = utils.NewAddressScriptHash(script,
					w.chainParams)
				return nil
			}
			return err
		...
		p2shAddr = addrInfo.Address().(*utils.AddressScriptHash)
		return nil
	})
	return p2shAddr, err
}
