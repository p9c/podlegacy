...
func makeMultiSigScript(w *wallet.Wallet, keys []string, nRequired int) ([]byte, error) {
	keysesPrecious := make([]*utils.AddressPubKey, len(keys))
	...
	for i, a := range keys {
		...
		default:
			...
			pubKeyAddr, err := utils.NewAddressPubKey(
...
func addMultiSigAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	secp256k1Addrs := make([]utils.Address, len(cmd.Keys))
...
func createMultiSig(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	address, err := utils.NewAddressScriptHash(script, w.ChainParams())
	if err != nil {
...
func decodeAddress(s string, params *chaincfg.Params) (utils.Address, error) {
	addr, err := utils.DecodeAddress(s, params)
	if err != nil {
...
func importPrivKey(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	wif, err := utils.DecodeWIF(cmd.PrivKey)
	if err != nil {
		return nil, &JSON.RPCError{
			Code:    JSON.ErrRPCInvalidAddressOrKey,
...
func getTransaction(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	var (
		debitTotal  utils.Amount
		creditTotal utils.Amount // Excludes change
		fee         utils.Amount
		feeF64      float64
	)
...
// and amounts.
func makeOutputs(pairs map[string]utils.Amount, chainParams *chaincfg.Params) ([]*wire.TxOut, error) {
	outputs := make([]*wire.TxOut, 0, len(pairs))
	for addrStr, amt := range pairs {
		addr, err := utils.DecodeAddress(addrStr, chainParams)
		if err != nil {
...
func sendPairs(w *wallet.Wallet, amounts map[string]utils.Amount,
	account uint32, minconf int32, feeSatPerKb utils.Amount) (string, error) {
...
func sendFrom(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	...
	amt, err := utils.NewAmount(cmd.Amount)
	if err != nil {
		return nil, err
	}
	pairs := map[string]utils.Amount{
		cmd.ToAddress: amt,
	}
...
func sendMany(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	pairs := make(map[string]utils.Amount, len(cmd.Amounts))
	for k, v := range cmd.Amounts {
		amt, err := utils.NewAmount(v)
		if err != nil {
			return nil, err
		}
		pairs[k] = amt
...
func signMessage(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	sigbytes, err := ecc.SignCompact(ecc.S256(), privKey,
		messageHash, true)
	if err != nil {
		return nil, err
	}
...
func signRawTransaction(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	...
	for _, rti := range cmdInputs {
		...
		if cmd.PrivKeys != nil && len(*cmd.PrivKeys) != 0 {
			...
			addr, err := utils.NewAddressScriptHash(redeemScript,
				w.ChainParams())
			if err != nil {
				return nil, DeserializationError{err}
			}
	...
	var keys map[string]*utils.WIF
	if cmd.PrivKeys != nil {
		keys = make(map[string]*utils.WIF)

		for _, key := range *cmd.PrivKeys {
			wif, err := utils.DecodeWIF(key)
			if err != nil {
				return nil, DeserializationError{err}
			}
			...
			addr, err := utils.NewAddressPubKey(wif.SerializePubKey(),
				w.ChainParams())
			if err != nil {
				return nil, DeserializationError{err}
			}
...
func verifyMessage(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	pk, wasCompressed, err := ecc.RecoverCompact(ecc.S256(), sig,
		expectedMessageHash)
	if err != nil {
		return nil, err
	}
	...
	case *utils.AddressPubKeyHash: // ok
		return bytes.Equal(utils.Hash160(serializedPubKey), checkAddr.Hash160()[:]), nil
	case *utils.AddressPubKey: // ok
