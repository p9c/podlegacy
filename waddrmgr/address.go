...
// Address returns the utils.Address which represents the managed address.
// This will be a pay-to-pubkey-hash address.
//
// This is part of the ManagedAddress interface implementation.
func (a *managedAddress) Address() utils.Address {
	return a.address
}
...
func (a *managedAddress) PrivKey() (*ecc.PrivateKey, error) {
	...
	privKey, _ := ecc.PrivKeyFromBytes(ecc.S256(), privKeyCopy)
...
func newManagedAddressWithoutPrivKey(m *ScopedKeyManager,
	...
	var pubKeyHash []byte
	if compressed {
		pubKeyHash = utils.Hash160(pubKey.SerializeCompressed())
	} else {
		pubKeyHash = utils.Hash160(pubKey.SerializeUncompressed())
	}
	...
	case NestedWitnessPubKey:
		...
		witAddr, err := utils.NewAddressWitnessPubKeyHash(
		...
		address, err = utils.NewAddressScriptHash(
	...
	case PubKeyHash:
		address, err = utils.NewAddressPubKeyHash(
			pubKeyHash, m.rootManager.chainParams,
		)
		if err != nil {
			return nil, err
	...
	case WitnessPubKey:
		address, err = utils.NewAddressWitnessPubKeyHash(
			pubKeyHash, m.rootManager.chainParams,
		)
		if err != nil {
			return nil, err
...
type scriptAddress struct {
	manager         *ScopedKeyManager
	account         uint32
	address         *utils.AddressScriptHash
...
//
// This is part of the ManagedAddress interface implementation.
func (a *scriptAddress) Address() utils.Address {
	return a.address
}
...
func newScriptAddress(m *ScopedKeyManager, account uint32, scriptHash,
	...
	address, err := utils.NewAddressScriptHashFromHash(
