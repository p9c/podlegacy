...
// newAddressPubKey returns a new utils.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) utils.Address {
	addr, err := utils.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new utils.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) utils.Address {
	addr, err := utils.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new utils.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) utils.Address {
	addr, err := utils.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the utils.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the utils.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := utils.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := utils.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := utils.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := utils.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := utils.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*utils.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*utils.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*utils.AddressPubKey)(nil), "", errUnsupportedAddress},
