...
// blockTemplateResult returns the current block template associated with the state as a btcjson.GetBlockTemplateResult that is ready to be encoded to JSON and returned to the caller.
// This function MUST be called with the state locked.
func (state *gbtWorkState) blockTemplateResult(useCoinbaseValue bool, submitOld *bool) (*btcjson.GetBlockTemplateResult, error) {
	...
	// Generate the block template reply.  Note that following mutations are implied by the included or omission of fields:
...
// handleGetNetworkHashPS implements the getnetworkhashps command. This command does not default to the same end block as the parallelcoind.
...
func handleSearchRawTransactions(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	// Add transactions from mempool first if client asked for reverse order.  Otherwise, they will be added last (as needed depending on the requested counts).
	// NOTE: This code doesn't sort by dependency.  This might be something to do in the future for the client's convenience, or leave it to the client.
	numSkipped := uint32(0)
...
func handleSendRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	// When the transaction was accepted it should be the first item in the returned array of accepted transactions.  The only way this will not be true is if the API for ProcessTransaction changes and this code is not properly updated, but ensure the condition holds as a safeguard.
...
type rpcserverConnManager interface {
	...
	// DisconnectByID disconnects the peer associated with the provided id. This applies to both inbound and outbound peers.  Attempting to
...
type rpcserverConfig struct {
	...
	// Generator produces block templates and the CPUMiner solves them using the CPU.  CPU mining is typically only useful for test purposes when doing regression or simulation testing.
