...
type NotificationHandlers struct {
	...
	OnRecvTx func(transaction *utils.Tx, details *JSON.BlockDetails)
	...
	//
	// NOTE: Deprecated. Use OnRelevantTxAccepted instead.
	OnRedeemingTx func(transaction *utils.Tx, details *JSON.BlockDetails)
	...
	// made to register for the notification and the function is non-nil.
	OnTxAccepted func(hash *chainhash.Hash, amount utils.Amount)
	...
	// such as btcwallet.
	OnAccountBalance func(account string, balance utils.Amount, confirmed bool)
...
func (c *Client) handleNotification(ntfn *rawNotification) {
	...
	// OnBtcdConnected
	case JSON.BtcdConnectedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
...
func parseFilteredBlockConnectedParams(params []json.RawMessage) (int32,
	*wire.BlockHeader, []*utils.Tx, error) {
	...
	transactions := make([]*utils.Tx, len(hexTransactions))
	for i, hexTx := range hexTransactions {
		...
		transactions[i], err = utils.NewTxFromBytes(transaction)
		if err != nil {
			return 0, nil, nil, err
		}
	}
...
// notifications.
func parseChainTxNtfnParams(params []json.RawMessage) (*utils.Tx,
	*JSON.BlockDetails, error) {
	...
	return utils.NewTx(&msgTx), block, nil
}
...
func parseTxAcceptedVerboseNtfnParams(params []json.RawMessage) (*JSON.TxRawResult,
	error) {
...
func parseAccountBalanceNtfnParams(params []json.RawMessage) (account string,
	balance utils.Amount, confirmed bool, err error) {
...
//
// NOTE: Deprecated. Use LoadTxFilterAsync instead.
func (c *Client) NotifyReceivedAsync(addresses []utils.Address) FutureNotifyReceivedResult {
	// Not supported in HTTP POST mode.
	if c.config.HTTPPostMode {
		return newFutureError(ErrWebsocketsRequired)
	}
...
//
// NOTE: Deprecated. Use LoadTxFilter instead.
func (c *Client) NotifyReceived(addresses []utils.Address) error {
...
func (c *Client) RescanEndBlockAsync(startBlock *chainhash.Hash,
	addresses []utils.Address, outpoints []*wire.OutPoint,
...
// NOTE: This is a btcd extension ported from github.com/decred/dcrrpcclient
// and requires a websocket connection.
func (c *Client) LoadTxFilterAsync(reload bool, addresses []utils.Address,
...
// NOTE: This is a btcd extension ported from github.com/decred/dcrrpcclient
// and requires a websocket connection.
func (c *Client) LoadTxFilter(reload bool, addresses []utils.Address, outPoints []wire.OutPoint) error {
