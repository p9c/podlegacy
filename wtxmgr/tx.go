...
func (s *Store) addCredit(ns walletdb.ReadWriteBucket, rec *TxRecord, block *BlockMeta, index uint32, change bool) (bool, error) {
	if block == nil {
		...
		v := valueUnminedCredit(utils.Amount(rec.MsgTx.TxOut[index].Value), change)
	...
	txOutAmt := utils.Amount(rec.MsgTx.TxOut[index].Value)
...
func (s *Store) rollback(ns walletdb.ReadWriteBucket, height int32) error {
	...
	for it.prev() {
		...
		for i := range b.transactions {
			...
			if chain.IsCoinBaseTx(&rec.MsgTx) {
				...
				for i, output := range rec.MsgTx.TxOut {
					...
					if credKey != nil {
						minedBalance -= utils.Amount(output.Value)
...
func (s *Store) UnspentOutputs(ns walletdb.ReadBucket) ([]Credit, error) {
	...
	err := ns.NestedReadBucket(bucketUnspent).ForEach(func(k, v []byte) error {
		...
		cred := Credit{
			...
			Amount:       utils.Amount(txOut.Value),
			PkScript:     txOut.PkScript,
			Received:     rec.Received,
			FromCoinBase: chain.IsCoinBaseTx(&rec.MsgTx),
		}
...
func (s *Store) Balance(ns walletdb.ReadBucket, minConf int32, syncHeight int32) (utils.Amount, error) {
	...
	for blockIt.prev() {
		...
		for i := range block.transactions {
			...
			for i := uint32(0); i < numOuts; i++ {
				...
				if confs < minConf || (chain.IsCoinBaseTx(&rec.MsgTx) &&
