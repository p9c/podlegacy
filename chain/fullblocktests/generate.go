...
func makeSpendableOutForTx(tx *wire.MsgTx, txOutIndex uint32) spendableOut {
	return spendableOut{
		...
		amount: utils.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := utils.Hash160(redeemScript)
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	...
	utilTxns := make([]*utils.Tx, 0, len(txns))
	for _, tx := range txns {
...
func additionalCoinbase(amount utils.Amount) func(*wire.MsgBlock) {
...
func additionalSpendFee(fee utils.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
// tests.
func createSpendTx(spend *spendableOut, fee utils.Amount) *wire.MsgTx {
...
func createSpendTxForTx(tx *wire.MsgTx, fee utils.Amount) *wire.MsgTx {
