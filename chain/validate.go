...
const (
	...
	baseSubsidy = 2 * utils.SatoshiPerBitcoin
)
...
func IsCoinBase(tx *utils.Tx) bool {
...
func IsFinalizedTransaction(tx *utils.Tx, blockHeight int32, blockTime time.Time) bool {
	msgTx := tx.MsgTx()
...
func CheckTransactionSanity(tx *utils.Tx) error {
	...
	for _, txOut := range msgTx.TxOut {
		...
		if totalSatoshi > utils.MaxSatoshi {
			str := fmt.Sprintf("total value of all transaction "+
...
func checkProofOfWork(header *wire.BlockHeader, powLimit *big.Int, flags BehaviorFlags) error {
	...
	if target.Cmp(pl) > 0 {
		str := fmt.Sprintf("block target difficulty of %064x is "+
			"higher than max of %064x", target, pl)
		return ruleError(ErrUnexpectedDifficulty, str)
	}
...
// difficulty is in min/max range and that the block hash is less than the
// target difficulty as claimed.
func CheckProofOfWork(block *utils.Block, powLimit *big.Int) error {
...
// txscript.
func CountSigOps(tx *utils.Tx) int {
	msgTx := tx.MsgTx()
...
func CountP2SHSigOps(tx *utils.Tx, isCoinBaseTx bool, utxoView *UtxoViewpoint) (int, error) {
...
func checkBlockSanity(block *utils.Block, powLimit *big.Int, timeSource MedianTimeSource, flags BehaviorFlags, DoNotCheckPow bool) error {
...
// CheckBlockSanity performs some preliminary checks on a block to ensure it is
// sane before continuing with block processing.  These checks are context free.
func CheckBlockSanity(block *utils.Block, powLimit *big.Int, timeSource MedianTimeSource, DoNotCheckPow bool) error {
...
func ExtractCoinbaseHeight(coinbaseTx *utils.Tx) (int32, error) {
...
func checkSerializedHeight(coinbaseTx *utils.Tx, wantHeight int32) error {
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) checkBlockContext(block *utils.Block, prevNode *blockNode, flags BehaviorFlags, DoNotCheckPow bool) error {
...
func (b *BlockChain) checkBIP0030(node *blockNode, block *utils.Block, view *UtxoViewpoint) error {
...
func CheckTransactionInputs(tx *utils.Tx, txHeight int32, utxoView *UtxoViewpoint, chainParams *chaincfg.Params) (int64, error) {
	...
	for txInIndex, txIn := range tx.MsgTx().TxIn {
		...
		if originTxSatoshi > utils.MaxSatoshi {
			str := fmt.Sprintf("transaction output value of %v is "+
		...
		if totalSatoshiIn < lastSatoshiIn ||
			totalSatoshiIn > utils.MaxSatoshi {
			str := fmt.Sprintf("total value of all transaction "+
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) checkConnectBlock(node *blockNode, block *utils.Block, view *UtxoViewpoint, stxos *[]SpentTxOut) error {
...
func (b *BlockChain) CheckConnectBlockTemplate(block *utils.Block) error {
	b.chainLock.Lock()
