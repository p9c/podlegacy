...
func TestFilterInsertKey(t *testing.T) {
	...
	wif, err := utils.DecodeWIF(secret)
	if err != nil {
	...
	f.Add(utils.Hash160(wif.SerializePubKey()))
...
func TestFilterBloomMatch(t *testing.T) {
	...
	tx, err := utils.NewTxFromBytes(strBytes)
	if err != nil {
	...
	spendingTx, err := utils.NewTxFromBytes(spendingTxBytes)
	if err != nil {
		t.Errorf("TestFilterBloomMatch NewTxFromBytes failure: %v", err)
		return
	}
