...
func (cs *CoinSet) TotalValue() (value utils.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue utils.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue utils.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount utils.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue utils.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue utils.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue utils.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue utils.Amount, coins []Coin) (Coins, error) {
...
// utils.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() utils.Amount {
	return utils.Amount(c.txOut().Value)
}
