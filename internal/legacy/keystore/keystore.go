...
import (
	...
	"github.com/parallelcointeam/pod/internal/legacy/rename"
...
func pubkeyFromPrivkey(privkey []byte, compress bool) (pubkey []byte) {
	_, pk := ecc.PrivKeyFromBytes(ecc.S256(), privkey)
...
func chainedPrivKey(privkey, pubkey, chaincode []byte) ([]byte, error) {
	...
	case ecc.PubKeyBytesLenUncompressed, ecc.PubKeyBytesLenCompressed:
		// Correct length
	default:
...
func chainedPubKey(pubkey, chaincode []byte) ([]byte, error) {
	var compressed bool
	switch n := len(pubkey); n {
	case ecc.PubKeyBytesLenUncompressed:
		compressed = false
	case ecc.PubKeyBytesLenCompressed:
		compressed = true
	default:
	...
	oldPk, err := ecc.ParsePubKey(pubkey, ecc.S256())
	if err != nil {
		return nil, err
	}
	newX, newY := ecc.S256().ScalarMult(oldPk.X, oldPk.Y, xorbytes)
	if err != nil {
		return nil, err
	}
	newPk := &ecc.PublicKey{
...
func getAddressKey(addr utils.Address) addressKey {
...
type Store struct {
	...
	passphrase       []byte
	secret           []byte
	chainIdxMap      map[int64]utils.Address
...
func New(dir string, desc string, passphrase []byte, net *chaincfg.Params,
	...
	s := &Store{
		...
		chainIdxMap:      make(map[int64]utils.Address),
...
func (s *Store) ReadFrom(r io.Reader) (n int64, err error) {
	...
	s.chainIdxMap = make(map[int64]utils.Address)
...
func (s *Store) NextChainedAddress(bs *BlockStamp) (utils.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
...
func (s *Store) ChangeAddress(bs *BlockStamp) (utils.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
...
func (s *Store) LastChainedAddress() utils.Address {
	s.mtx.RLock()
	defer s.mtx.RUnlock()
...
func (s *Store) Address(a utils.Address) (WalletAddress, error) {
	s.mtx.RLock()
	defer s.mtx.RUnlock()
...
func (s *Store) SetSyncStatus(a utils.Address, ss SyncStatus) error {
	s.mtx.Lock()
	defer s.mtx.Unlock()
...
func (s *Store) ImportPrivateKey(wif *utils.WIF, bs *BlockStamp) (utils.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
	...
	pkh := utils.Hash160(wif.SerializePubKey())
...
func (s *Store) ImportScript(script []byte, bs *BlockStamp) (utils.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
	...
	if _, ok := s.addrMap[addressKey(utils.Hash160(script))]; ok {
		return nil, ErrDuplicate
	}
...
type WalletAddress interface {
	// Address returns a utils.Address for the backing address.
...
func (s *Store) ActiveAddresses() map[utils.Address]WalletAddress {
	s.mtx.RLock()
	defer s.mtx.RUnlock()

	addrs := make(map[utils.Address]WalletAddress)
	for i := int64(rootKeyChainIdx); i <= s.highestUsed; i++ {
		a := s.chainIdxMap[i]
...
//
// A slice is returned with the utils.Address of each new address.
// The blockchain must be rescanned for these addresses.
func (s *Store) ExtendActiveAddresses(n int) ([]utils.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
...
func newBtcAddressWithoutPrivkey(s *Store, pubkey, iv []byte, bs *BlockStamp) (addr *btcAddress, err error) {
	...
	pk, err := ecc.ParsePubKey(pubkey, ecc.S256())
	if err != nil {
		return nil, err
	}

	address, err := utils.NewAddressPubKeyHash(utils.Hash160(pubkey), s.netParams())
	if err != nil {
		return nil, err
	}
...
func (a *btcAddress) ReadFrom(r io.Reader) (n int64, err error) {
	...
	addr, err := utils.NewAddressPubKeyHash(pubKeyHash[:], a.store.netParams())
	if err != nil {
		return n, err
	}
	a.address = addr
...
func (a *btcAddress) unlock(key []byte) (privKeyCT []byte, err error) {
	...
	x, y := ecc.S256().ScalarBaseMult(privkey)
...
func (a *btcAddress) Address() utils.Address {
	return a.address
}
...
// PubKeyAddress.
func (a *btcAddress) PubKey() *ecc.PublicKey {
	return a.pubKey
}
...
func (a *btcAddress) PrivKey() (*ecc.PrivateKey, error) {
...
func (a *btcAddress) ExportPrivKey() (*utils.WIF, error) {
	...
	// elsewhere.
	wif, err := utils.NewWIF((*ecc.PrivateKey)(pk), a.store.netParams(),
		a.Compressed())
	if err != nil {
		panic(err)
	}
...
func newScriptAddress(s *Store, script []byte, bs *BlockStamp) (addr *scriptAddress, err error) {
	...
	scriptHash := utils.Hash160(script)

	address, err := utils.NewAddressScriptHashFromHash(scriptHash, s.netParams())
	if err != nil {
		return nil, err
	}
...
func (sa *scriptAddress) ReadFrom(r io.Reader) (n int64, err error) {
	...
	address, err := utils.NewAddressScriptHashFromHash(scriptHash[:],
		sa.store.netParams())
	if err != nil {
		return n, err
	}
...
// address returns a utils.AddressScriptHash for a btcAddress.
func (sa *scriptAddress) Address() utils.Address {
	return sa.address
}
...
func (sa *scriptAddress) Addresses() []utils.Address {
	return sa.addresses
}
